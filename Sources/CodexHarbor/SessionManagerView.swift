        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            List(selection: $selected) {
                ForEach(groupedKeys, id: \.self) { key in
                    DisclosureGroup { ForEach(model.sessions.filter { ($0.group ?? "未分组") == key && (filter == "全部" || key == filter) }) { s in HStack { Image(systemName: s.deleted ? "trash" : "bubble.left.and.bubble.right"); Text(s.title).lineLimit(1); Spacer(); if s.deleted { Text("回收站").font(.caption2).foregroundStyle(.secondary) } }.tag(s.id).onDrag { NSItemProvider(object: s.id as NSString) }.contextMenu { Button("编辑名称") { renameID = s.id; renameText = s.title }; Button("移入回收站", role: .destructive) { Task { await model.deleteSessions([s.id]) } }; Button("恢复") { Task { await model.restoreSessions([s.id]) } }; Button("取消分组") { Task { await model.groupSessions([s.id], group: nil) } } } } } label: { Label(key, systemImage: "folder") .onDrop(of: [.text], isTargeted: nil) { providers in for provider in providers { provider.loadObject(ofClass: NSString.self) { object, _ in if let id = object as String? { Task { await model.groupSessions([id], group: key) } } } }; return true } }
                }
            }
