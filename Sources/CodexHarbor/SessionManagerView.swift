        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            List(selection: $selected) {
                ForEach(groupedKeys, id: \.self) { key in
                    DisclosureGroup {
                        ForEach(model.sessions.filter { ($0.group ?? "未分组") == key && (filter == "全部" || key == filter) }) { s in sessionRow(s) }
                    } label: {
                        Label(key, systemImage: "folder")
                            .onDrop(of: [.text], isTargeted: nil) { providers in
                                for provider in providers { provider.loadObject(ofClass: NSString.self) { object, _ in if let id = object as String? { Task { await model.groupSessions([id], group: key) } } } }
                                return true
                            }
                    }
                }
