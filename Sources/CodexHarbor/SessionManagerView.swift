...
struct SessionManagerView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            List(model.sessions.filter { filter == "全部" || ($0.group ?? "未分组") == filter }, selection: $selected) { s in HStack { Image(systemName: s.deleted ? "trash" : "bubble.left.and.bubble.right"); Text(s.title).lineLimit(1); Spacer(); Text(s.group ?? "未分组").font(.caption).foregroundStyle(.secondary) }.tag(s.id) }
