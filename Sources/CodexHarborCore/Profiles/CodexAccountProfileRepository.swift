...
public actor CodexAccountProfileRepository {
    ...
    private func saveAuthentication(
        ...
        if let index = catalog.profiles.firstIndex(where: { $0.credentialFingerprint == fingerprint }) {
            ...
            catalog.profiles[index].subscriptionLastCheckedAt = subscription.checkedAt
            if markUsed {
            ...
            profile = CodexAccountProfile(
                ...
                subscriptionLastCheckedAt: subscription.checkedAt
            )
    ...
    private nonisolated static func subscriptionDetails(in data: Data) -> (plan: String?, expiresAt: String?, checkedAt: String?) {
        ...
        let idChecked = subscriptionCheckedDate(in: idAuth)
        let accessChecked = subscriptionCheckedDate(in: accessAuth)
        let accessIsNewer = accessChecked.map { access in
            idChecked.map { access > $0 } ?? true
        } ?? false
        let primary = accessIsNewer ? accessAuth : idAuth
        let fallback = accessIsNewer ? idAuth : accessAuth
        let plan = (primary?["chatgpt_plan_type"] as? String)
            ?? (fallback?["chatgpt_plan_type"] as? String)
        let expiry = (primary?["chatgpt_subscription_active_until"] as? String)
            ?? (fallback?["chatgpt_subscription_active_until"] as? String)
        let checkedAt = (primary?["chatgpt_subscription_last_checked"] as? String)
            ?? (fallback?["chatgpt_subscription_last_checked"] as? String)
        return (plan, expiry, checkedAt)
    }

    private nonisolated static func subscriptionCheckedDate(in claims: [String: Any]?) -> Date? {
        guard let value = claims?["chatgpt_subscription_last_checked"] as? String else { return nil }
